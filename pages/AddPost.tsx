...
const AddPost: React.FC = () => {
  ...
  const baseURL = `${typeof window !== 'undefined' ? window.location.origin : ''}/${contentType}s/`;
  ...
  useEffect(() => {
    ...
    const basePath = router.pathname.split('/')[1]; // Use 'let' to allow reassignment
  ...
  }, [contentTypes, contentLoading, router.pathname]);
  useEffect(() => {
  ...
  // Separate useEffect for loading post data when editing
  useEffect(() => {
    if (!slug) return; // Only run when there's a slug (editing mode)
    ...
    loadPostData();
  }, [slug]); // Only depends on slug to avoid infinite loops
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (newPostId) {
        ...
        // Navigate immediately without fetching updated post data to avoid infinite loops
        if (slug) {
          // If editing, stay on the same page
          setGalleryImages([]);
        } else {
          // If creating new, navigate to edit page
          router.push(`/${contentType}s/${response.slug}/edit`);
        }
      } else {
        console.error(
          `${capitalizeFirstLetter(
