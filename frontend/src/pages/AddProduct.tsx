...
const AddProduct: React.FC = () => {
  ...
  const [shortDescription, setShortDescription] = useState<string>(''); // State for short description
  ...
  useEffect(() => {
    ...
    const loadCategories = async () => {
      try {
        ...
        if (slug) {
          const fetchedPost = await fetchProductBySlug(slug);
          ...
          setAdditionalInfo(fetchedPost.additional_info);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (slug) {
        response = await updateProduct(slug, postData);
      } else {
      ...
      // Upload the selected featured images if they exist
                ...
                {isEditingSlug ? (
                  <div className="flex items-center ml-2">
                    <input
                      ...
                      className="appearance-none border rounded p-2 w-72 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                    />
                    <button
                      type="button"
                      onClick={() => {
          ...
          <div className="mt-6">
            <div className="mb-6">
              ...
              <input
                id="price"
                type="number"
                value={price ?? ''} // Bind price state
                onChange={(e) => setPrice(Number(e.target.value))} // Update price state
                className="appearance-none border rounded w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                placeholder="Price"
              />
            </div>
